...
?>
   ...
   <div class="admin-content">
    ...
    <div class="content">
        <h2 class="page-title">تعديل منشور</h2>

    <div>
    <form action="posts.php?id_new=<?php echo $post_id; ?>" method="post">
        <div>
            <input type="hidden" name="post_id" value="<?php echo $row['post_id']; ?>">
        </div>
    <div>
        <label>عنوان المنشور
        <input type="text" name="title" class="text-input" maxlength="40" value="<?php echo $row['title']; ?>" required>
        </label>
    </div>

    <div>
            <label>الكاتب
            <input type="text" name="author" class="text-input" maxlength="20" value="<?php echo $row['author']; ?>" required>
            </label>
        </div>

    <div>
        <label>محتوى المنشور
        <textarea type="text" name="content" id="editor" class="text-input" required>
        </textarea>
        </label>
    </div>
